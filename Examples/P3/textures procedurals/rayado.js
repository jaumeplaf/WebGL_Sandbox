...
function initShaders() { 
  ...
  // propia del shader
  program.StripeColorIndex      = gl.getUniformLocation( program, "StripeColor");
  ...
  gl.uniform1f(program.StripeWidthIndex, 0.0);
  gl.uniform1f(program.StripeScaleIndex, 1.0);
  gl.uniform3f(program.StripeColorIndex, 1.0, 1.0, 1.0);
...
function initMyHandlers() {
  ...
  for (var i = 0; i < colors.length; i++) {
    ...
    function(){
      switch (this.getAttribute("id")) {
        case "Kd":          setColor (program.KdIndex,          this.value); break;
        case "StripeColor": setColor (program.StripeColorIndex, this.value); break;
      }
      requestAnimationFrame(drawScene);
    },
    false);
  ...
  for (var i = 0; i < ranges.length; i++) {
    ...
    function(){
      switch (this.getAttribute("id")) {
        case "StripeWidth": gl.uniform1f(program.StripeWidthIndex, this.value/100.0); break;
        case "StripeScale": gl.uniform1f(program.StripeScaleIndex, this.value);       break;
      }
      requestAnimationFrame(drawScene);
    },
    false);
