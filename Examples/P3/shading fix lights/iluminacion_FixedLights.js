...
function setShaderModelMatrix(modelMatrix) {
...
var POIs = [];
var lightPos = [10.0, 10.0, 0.0, 1.];

// retorna camera modificada 
function moveEye () {
...
function setShaderLight() {
  ...
  //gl.uniform3f(program.PositionIndex, 10.0, 10.0, 0.0); // en coordenadas del ojo: original
  gl.uniform3f(program.PositionIndex, lightPos[0], lightPos[1], lightPos[2]); // en coordenadas del ojo
}

// calcula transformada de la llum 
function setTransformLight() {
  Lp = vec4.create();
...
function drawScene() {
  ...
  mat4.multiply    (modelViewMatrix, moveEye(), modelMatrix);
