...
var gl, program;
var myTorus, mycylinder;
var nsides = 6;
var myZeta = 0.0, myPhi = Math.PI/2.0, radius = 1.4, fovy = 1.4;
var selectedPrimitive = exampleCube2;
...
function setShaderLight() {
  ...
  //gl.uniform3f(program.PositionIndex, 10.0, 10.0, 10.0); // en coordenadas del ojo
...
function drawScene() {
  ...
  var T = mat4.create();
  mat4.fromScaling (modelMatrix, [4, 2, 4]);
  // traslada el modelo: test
  mat4.fromTranslation(T,[0,0,0]);
  ...
  // calcula transformada de la llum 
  Lp = vec4.create();
  mat4.multiply(Lp, getCameraMatrix(),Lpos);

  gl.uniform3f(program.PositionIndex, Lp[0], Lp[1], Lp[2]);
  //gl.uniform3f(program.PositionIndex, Lpos[0], Lpos[1], Lpos[2]);
  ...
  // dibuja llum
  mat4.fromScaling (modelMatrix, [0.05, 0.05, 0.05]);
  mat4.fromTranslation(T,Lpos);
  mat4.multiply (modelMatrix, T, modelMatrix);
  mat4.multiply    (modelViewMatrix, getCameraMatrix(), modelMatrix);
  setShaderModelViewMatrix(modelViewMatrix);
  setShaderNormalMatrix( getNormalMatrix(modelViewMatrix) );
  ...
  for (var i = 0; i < botones.length; i++) {
    ...
    function(){
      switch (this.innerHTML) {
        case "Cubo":     selectedPrimitive = exampleCube2;     break;
      }
      requestAnimationFrame(drawScene);
    },
    false);
